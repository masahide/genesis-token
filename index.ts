...
import { BitboxNetwork, SlpBalancesResult, Utils } from "slpjs";
...
(async () => {
    ...
    const fundingWif = BITBOX.HDNode.toWIF(change) // <-- compressed WIF format
    //const bchChangeReceiverAddress = cashAddress
    const bchChangeReceiverAddress = slpAddress
    ...
    const tokenReceiverAddress     = "simpleledger:自分のアドレス";  // <-- must be simpleledger format
    ...
    const batonReceiverAddress     = tokenReceiverAddress;
